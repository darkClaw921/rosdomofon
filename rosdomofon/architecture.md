...
- **Преобразование `flat_id` в int** в `CreateConnectionRequest` - API РосДомофон ожидает `flat_id` как число, поэтому строки преобразуются в int (для совместимости с API, который возвращает ID как строки)
- **Явное преобразование типа в `connect_service`** - метод `connect_service` явно преобразует `flat_id` в int перед созданием модели и использует `model_dump(mode='json')` для правильной JSON сериализации с сохранением типов
- **Логирование тела запроса и ответа** - при ошибках 400 и 422 логируется тело ответа API для диагностики проблем
...
- **Класс RosDomofonAPI** с методами:
  ...
  - `connect_service()` - подключение услуги (принимает flat_id как int или str, преобразуется в int)
...
- **Класс RosDomofonKafkaClient** с методами:
  ...
  - `fetch_latest_signups()` - единоразовое получение последних регистраций без фонового потребителя (поддерживает общий и корпоративный топики)
...
- Возможность получить свежий срез регистраций без влияния на основную consumer группу (`fetch_latest_signups()`)
